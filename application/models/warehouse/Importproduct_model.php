...
$data2['date_end2'] = time()+622080000;
}
...
$this->db->insert("wh_importproduct_detail", $data2);
...
$querynum = $this->db->query('SELECT *,
    ...
    WHERE branch_id="'.$_SESSION['branch_id'].'"  AND importproduct_header_refcode LIKE "%'.$data['searchtext'].'%"
    OR branch_id="'.$_SESSION['branch_id'].'"  AND importproduct_header_remark LIKE "%'.$data['searchtext'].'%"
    OR branch_id="'.$_SESSION['branch_id'].'"  AND importproduct_header_code LIKE "%'.$data['searchtext'].'%"
    ORDER BY importproduct_header_id DESC ');


$query = $this->db->query('SELECT *,
    from_unixtime(importproduct_header_date,"%d-%m-%Y %H:%i:%s") as importproduct_header_date2
	FROM wh_importproduct_header as wh
...
return $json;
     ...
     WHERE wd.branch_id="'.$_SESSION['branch_id'].'" AND  wd.date_end !="" AND wd.status="0"  AND wl.product_name LIKE "%'.$data['searchtext'].'%"
     OR wd.branch_id="'.$_SESSION['branch_id'].'" AND  wd.date_end !="" AND wd.status="0" AND wd.lotno LIKE "%'.$data['searchtext'].'%"
	 OR wd.branch_id="'.$_SESSION['branch_id'].'" AND  wd.date_end !="" AND wd.status="0" AND wd.importproduct_header_code LIKE "%'.$data['searchtext'].'%"
	 OR wd.branch_id="'.$_SESSION['branch_id'].'" AND  wd.date_end !="" AND wd.status="0" AND wl.product_code LIKE "%'.$data['searchtext'].'%"
...
$this->db->query('UPDATE wh_importproduct_header
             SET confirm="1"
...
$query_ss = $this->db->query('SELECT * FROM stock WHERE product_id="'.$product_id.'" and  branch_id="'.$_SESSION['branch_id'].'"');
$num_rows_ss = $query_ss->num_rows();

if($num_rows_ss > 0){
	
$this->db->query('UPDATE stock SET product_stock_num=product_stock_num + '.$numnow.'
WHERE product_id="'.$product_id.'" and  branch_id="'.$_SESSION['branch_id'].'"');
	
}else{ 
$data_ss['product_id'] = $product_id;
$data_ss['branch_id'] = $_SESSION['branch_id'];
$data_ss['product_stock_num'] = $numnow;
