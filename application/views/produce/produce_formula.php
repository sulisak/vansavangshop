...
<input type="text" ng-model="title" placeholder="<?php echo $lang_pdpf_1;?>" class="form-control" style="width: 200px;">
</div>
<div class="form-group">
<input class="form-control" style="width: 500px;" ng-model="des" placeholder="<?php echo $lang_detail;?>">
</div>
</form>
...
<div class="form-group">
				<input type="text" class="form-control" id="product_name_title" ng-model="productmain[0].product_name_title" style="font-size: 20px;text-align: right;height: 47px;width: 300px;background-color:#dff0d8;" 
				placeholder="<?php echo $lang_pdpf_1_1;?>" disabled>
				</div>
				
				
				<div class="form-group">
<button ng-click="Searchproduct()" class="btn btn-primary btn-lg" placeholder="" title="">
	<?php echo $lang_pdpf_2;?></button> 
</div>


<div class="form-group">
<button ng-click="Searchproductmat()" class="btn btn-primary btn-lg" placeholder="" title="">
	<?php echo $lang_pdpf_3;?></button>
</div>
...
<thead>
	...
	<th style="text-align: center;width: 50px;"><?=$lang_rank?></th>
		<th style="text-align: center;"><?php echo $lang_pdpf_4;?></th>
...
{{x.product_name_mat}}
			...
			<input type="hidden" ng-model="x.product_id_mat">
			</td>
			...
			<td>
				<input style="text-align: right;" type="text" ng-model="x.product_num_mat" class="form-control" placeholder="<?=$lang_unit?>">
			</td>
			<td>
{{x.product_unit_name_mat}}
			</td>
			<!-- <td>
...
placeholder="<?php echo $lang_pdpf_1_2;?>" style="width: 300px;">
</div>
...
<th style="text-align: center;"><?php echo $lang_pdpf_1;?></th>


			<th style="text-align: center;"><?php echo $lang_pdpf_5;?></th>
			...
			<th style="text-align: center;"> <?php echo $lang_pdpf_6;?></th>
	...
	<tbody>
		<tr ng-repeat="x in list">
			...
			<td align="center">
			{{x.produce_formula_no}} 
			<button class="btn btn-warning" ng-click="Getimportone(x)">
			<?php echo $lang_pdpf_7;?>
			</button></td>
			<td align="center">{{x.produce_formula_name}}</td>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title"><?php echo $lang_pdpf_8;?></h4>
			</div>
...
<table class="table table-hover">
	...
	<tbody>
		<tr ng-repeat="x in productlist" ng-if="x.pid==null">
...
<div class="modal fade" id="Searchproductmat">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h4 class="modal-title"><?php echo $lang_pdpf_9;?></h4>
			</div>
			<div class="modal-body">

<form class="form-inline">
<div class="form-group">
<input type="text" ng-model="product_name" ng-change="Searchproductfind()" class="form-control" placeholder="ค้นหาชื่อสินค้า" style="height: 45px;font-size: 20px;">
</div>
...
<table class="table table-hover">
	...
	<tbody>
		<tr ng-repeat="x in productlist">
			<td style="text-align:center;"><button class="btn btn-success" ng-click="Addpushproductcodemat(x.product_code)">
<?php echo $lang_select;?>			</button></td>
			<td style="text-align:center;">{{x.product_name}}</td>
		</tr>
	</tbody>
...
<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
	...
	<b><?php echo $lang_pdpf_10;?>:: {{pfheader.produce_formula_name}}</b>
	...
	<tr>
	<td align="right">No:</td><td>{{pfheader.produce_formula_no}}</td>
	<td align="right"> <?php echo $lang_pdpf_11;?>:</td><td>{{pfheader.adddate}}</td>
	</tr>
	<tr>
	<td align="right"><?php echo $lang_pdpf_12;?>:</td><td>{{pfheader.product_name}}</td>
	<td align="right"><?php echo $lang_detail;?>:</td><td>{{pfheader.des}}</td></tr>
</table>
...
<thead>
	<tr class="trheader">
	<th style="text-align: center;width: 50px;"><?=$lang_rank?></th>
		<th style="text-align: center;"><?php echo $lang_pdpf_13;?></th>
		<th style="text-align: center;"><?php echo $lang_barcode;?></th>
...
{{x.product_unit_name_mat}}
		...
		<tr>
			<td colspan="3" align="right"><?=$lang_all?></td>
			<td align="right" style="font-weight: bold;">{{Sumnum2() | number}}</td>
...
$scope.Searchproductmat = function(){

$('#Searchproductmat').modal('show');
...
$http.post("produce_formula/Findproduct2",{
...
$scope.Addpushproductcode = function(product_code){
	$scope.productmain = [];
$http.post("produce_formula/Findproduct",{
...
$scope.Addpushproductcodemat = function(product_code){
$http.post("produce_formula/Findproduct",{
	product_code: product_code
	}).success(function(data){
		$scope.Findproductone = data;
...
$scope.productimportlist.push({
	product_id_mat: data[0].product_id,
	product_code_mat: data[0].product_code,
	product_name_mat: data[0].product_name,
	product_unit_name_mat: data[0].product_unit_name,
	product_num_mat: '1'
});
$scope.cannotfindproduct = false;
}
...
total += parseFloat(item.product_num_mat);
 });
    return total;
};

$scope.Sumnum2 = function(){
var total = 0;

 angular.forEach($scope.pfdetail,function(item){
total += parseFloat(item.product_num_mat);
 });
    return total;
};
...
$scope.Saveimportproduct = function(){
	if($scope.productimportlist!=''){
		...
		if($scope.title!='' && $scope.productmain[0].product_name!='' && $scope.productimportlist[0].product_id!='' && $scope.productimportlist[0].importproduct_detail_num!='0'){

$('#Saveimportproduct').prop('disabled',true);
$('#Saveimportproduct2').prop('disabled',true);
$('#product_code').prop('disabled',true);
$http.post("produce_formula/add",{
	produce_formula_name: $scope.title,
	des: $scope.des,
	product_id: $scope.productmain[0].product_id,
	product_name: $scope.productmain[0].product_name,
	product_code: $scope.productmain[0].product_code,
...
$http.post("produce_formula/Getimportone",{
	produce_formula_no: x.produce_formula_no
}).success(function(response){
$scope.pfdetail = response;
$scope.pfheader = x;
        });
...
$http.post("produce_formula/Deleteimportlist",{
	produce_formula_no: x.produce_formula_no
}).success(function(response){
$scope.getlist();
