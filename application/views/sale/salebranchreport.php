...
<center>
	<b><h1><?php echo $lang_sbrp_1;?></b></h1>
...
<table ng-if="daylist" id="headerTable" class="table table-hover table-bordered">
	<thead>
		...
		<th style="text-align: center;"><?php echo $lang_branch;?></th>
			<th style="text-align: center;width: 150px;"><?php echo $lang_qty;?></th>
	...
	<tbody>
		...
		<td>{{$index+1}}</td>
		<td><button class="btn btn-warning" ng-click="Opensalelistdatail(x)">{{x.branch_name}}</button></td>
...
<center><h2 class="modal-title"><?php echo $lang_sbrp_2;?> {{data_cus_name}}</h2>
	{{dayfrom}} <?php echo $lang_to;?> {{dayto}}
	</center>
<table class="table table-hover table-bordered">
	<thead>
		<tr class="trheader">
			...
			<th class="text-right"><?php echo $lang_sbrp_3;?></th>
...
$scope.data_cus_name = x.branch_name;

$http.post("Salebranchreport/Salelistdatail",{
...
$scope.reportdaylist = function(){
	$scope.daylist = false;
$http.post("Salebranchreport/daylist",{
	dayfrom: $scope.dayfrom,
	dayto: $scope.dayto,
	searchtext: $scope.searchtext
	}).success(function(data){
...
$scope.datac.push({count: item.saleprice,name: item.branch_name});
});
