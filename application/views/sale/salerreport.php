...
<input type="text" name="" placeholder="<?=$lang_search?> <?php echo $lang_srrp_1;?>" ng-model="searchtext" ng-change="reportdaylist()" class="form-control">
</div>
<div class="form-group">
...
<center>
	<b><h1><?php echo $lang_srrp_2;?></b></h1>
...
<table ng-if="daylist" id="headerTable" class="table table-hover table-bordered">
	<thead>
		...
		<th style="text-align: center;"><?php echo $lang_srrp_3;?></th>
			<th style="text-align: center;width: 150px;"><?php echo $lang_srrp_4;?></th>
			<th style="text-align: center;width: 150px;"><?php echo $lang_allsaleprice;?></th>
			<th style="text-align: center;width: 150px;"><?php echo $lang_discountlast;?></th>
	...
	<tbody>
		...
		<td>{{$index+1}}</td>
		<td><button class="btn btn-warning" ng-click="Opensalelistdatail(x)">{{x.name}}</button></td>
...
<center><h2 class="modal-title"><?php echo $lang_srrp_5;?> {{data_cus_name}}</h2>
...
$scope.data_cus_name = x.name;

$http.post("Salerreport/Salelistdatail",{
...
$scope.datac.push({count: item.saleprice,name: item.name});
});
...
Morris.Bar({
  ...
  xkey: 'name',
  ykeys: ['count'],
  labels: ['<?php echo $lang_allsaleprice;?>'],
