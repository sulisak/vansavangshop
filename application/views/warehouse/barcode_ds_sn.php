	...
	<div class="panel panel-default">
		<div class="panel-body">
			...
			<div id="openprint1" >
				...
				<?php
foreach ($sn_code as $value) {
	?>
	
	
				<div  style="margin-right: {{marginright/2}}px;margin-left: {{marginright/2}}px;margin-bottom: {{marginbottom}}px;margin-top: {{margintop}}px;overflow: hidden;float: left;text-align:left;outline:{{outline}}px dotted;">
					...
					<center style="padding-left: 10px;padding-right: 10px;">
						...
						<?php
							...
							//echo file_get_contents($base_url.'/bc/c2mbarcode.php?barcode='.$value["sn_code"]);
							
							
  	  	if($_GET['type']=='1'){

    echo '<img width="200px" height="50" src="'.$base_url.'/bc/c2mbarcode.php?barcode='.$value["sn_code"].'">';
echo '<br /><b style="font-size: {{sizepname}}px;">'.$value["sn_code"].'</b><br />';
	}

if($_GET['type']=='0'){

    echo '<img width="300px" height="100" src="'.$base_url.'/warehouse/barcode/png?barcode='.$value["sn_code"].'">';
echo '<br /><b>'.$value["sn_code"].'</b><br />';
	}
	
	if($_GET['type']=='2'){
		echo '<img src="'.$base_url.'/qr/?code='.$value["sn_code"].'">';
		echo '<br /><b>'.$value["sn_code"].'</b><br />';
	}
